...
Flow:
  Raw CSVs (folder)
    ...
    -> [Step 1] build_session_table()    aggregate per charging session
    -> [Step 2] compute_soh_labels()       energy-based pseudo-labels
    -> [Step 3] train_xgboost_soh()        XGBoost SOH per session
    -> [Step 4] train_lstm_trajectory()    LSTM SOH trajectory
    -> [Step 5] compute_all_rul()          RUL (days to 80% EOL)
...
MIN_DELTA_SOC       = 2.0             # Minimum SOC swing % to include a session at all (display gate)
MIN_DELTA_SOC_LABEL = 2.0             # Minimum SOC swing % to use a session for soh_label computation (quality gate)
                                      # Set equal to MIN_DELTA_SOC so all display-gate sessions get real labels.
                                      # (Setting this higher, e.g. 10%, creates NaN label gaps that cause linear
                                      # interpolation artifacts and monotone anchoring bias — keep equal to MIN_DELTA_SOC.)
MIN_AH        = 1.0                   # Minimum Ah delivered in a session
EXPECTED_CAPACITY_OPTIONS_AH = (104.5, 153.0, 306.0, 608.0)  # Fleet pack capacities
...
# ------------------------------------------------------------------------------
Q_RATED_AH    = 306.0                 # Fallback nominal capacity floor
...
PACK_CAPACITY_OPTIONS_BY_SERIES = {
    96: (104.5,),
    120: (104.5,),
    208: (153.0, 306.0, 608.0),      # 208s1p / 208s2p / 208s4p
}
...
PACK_208_FORCE_4P_Q_THRESHOLD_AH = 400.0
...
CONFIG_PROFILES = {
    'default': {
        ...
        'MIN_DELTA_SOC': 1.0,
        'MIN_AH': 1.0,
        'EXPECTED_CAPACITY_OPTIONS_AH': [104.5, 153.0, 306.0, 608.0],
...
def _validate_runtime_config():
    ...
    if float(MIN_DELTA_SOC_LABEL) < float(MIN_DELTA_SOC):
        raise ValueError(f"Invalid MIN_DELTA_SOC_LABEL={MIN_DELTA_SOC_LABEL}. Must be >= MIN_DELTA_SOC ({MIN_DELTA_SOC}).")
    if float(MIN_AH) <= 0.0:
...
def _capacity_options_from_pack_config(cfg_text: str):
    ...
    if np.isfinite(s_cells) and np.isfinite(p_count) and int(s_cells) == 208:
        if int(p_count) >= 4:
            return (608.0,)
        return (306.0,) if int(p_count) >= 2 else (153.0,)
...
def _calibrate_capacity_ah(q_raw, options=None, scales=None):
    ...
    return q_cal, nom, scale, err * 100.0
...
def _infer_pack_config_options(g: pd.DataFrame, q_anchor=np.nan, q_prev=np.nan):
    ...
    # Stricter gate for capacity evidence: 10% delta_soc reduces amplification noise 2×
    # compared to 5%, and excludes flagged outlier sessions. Falls back to 5% gate if
    # fewer than 8 sessions qualify at 10% (prevents data starvation on top-up-only vehicles).
    q_quality = quality.copy()
    if 'delta_soc_pct' in g.columns:
        q_quality &= _finite_series(g['delta_soc_pct']) >= 10.0
    if 'implied_Q_outlier' in g.columns:
        q_quality &= ~g['implied_Q_outlier'].fillna(False)
    ...
    q_q = _finite_series(g['implied_Q_Ah'].where(q_quality)) if 'implied_Q_Ah' in g.columns else pd.Series(dtype=float)
    q_q = q_q.where(q_q > 0)
    # Fallback to 5% gate if the 10% gate leaves fewer than 8 sessions
    if int(q_q.notna().sum()) < 8 and 'implied_Q_Ah' in g.columns:
        q_q = _finite_series(g['implied_Q_Ah'].where(quality)).where(lambda s: s > 0)
        if 'implied_Q_outlier' in g.columns:
            q_q = q_q.where(~g['implied_Q_outlier'].fillna(False))
...
def _estimate_initial_capacity_ah(g: pd.DataFrame, first_cycles: int = INIT_CAPACITY_CYCLES) -> dict:
    ...
    if 'delta_soc_pct' in early.columns:
        mask &= _finite_series(early['delta_soc_pct']) >= 10.0
    ...
    if 'implied_Q_outlier' in early.columns:
        mask &= ~early['implied_Q_outlier'].fillna(False)

    q_early = _finite_series(early.loc[mask, 'implied_Q_Ah'])
    if q_early.notna().sum() < 4:
        # Relax delta_soc gate to 5% when insufficient sessions qualify at 10%.
        ...
        if 'implied_Q_outlier' in early.columns:
            relaxed &= ~early['implied_Q_outlier'].fillna(False)
...
def _read_csv_resilient(path: Path) -> pd.DataFrame:
    ...
    keep_cols = {
        ...
        'temp_spread_mean', 'implied_Q_Ah', 'implied_Q_outlier', 'soh_label', 'soh_smooth',
...
def segment_sessions(
    ...
    Logic is identical to the notebook    only the loop is index-based
...
def build_session_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Flag sessions whose implied_Q_Ah is outside 0.6×–1.4× the nearest known nominal.
    # Small delta_soc sessions (<10%) amplify current-sensor noise up to 50×, producing
    # implied_Q_Ah values that are physically implausible. These sessions are kept in the
    # output (customers can inspect them) but excluded from baseline / pack-config estimation.
    _q_opts = tuple(float(v) for v in EXPECTED_CAPACITY_OPTIONS_AH)
    if _q_opts:
        _nearest_nom = sessions['implied_Q_Ah'].apply(
            lambda q: min(_q_opts, key=lambda x: abs(x - q))
            if np.isfinite(q) and q > 0 else np.nan
        )
        sessions['implied_Q_outlier'] = (
            sessions['implied_Q_Ah'].notna() &
            _nearest_nom.notna() &
            (
                (sessions['implied_Q_Ah'] < _nearest_nom * 0.60) |
                (sessions['implied_Q_Ah'] > _nearest_nom * 1.40)
            )
        )
    else:
        sessions['implied_Q_outlier'] = False
...
def compute_soh_labels(
    ...
    # Session quality filter — all sessions passing this gate get a real soh_label.
    # Keeping MIN_DELTA_SOC_LABEL = MIN_DELTA_SOC ensures dense labels (no NaN gaps),
    # which gives a stable rolling median and prevents monotone anchoring artifacts.
    base_mask = (
    ...
    print(f"      - delta_SOC >= {MIN_DELTA_SOC}%  : {base_mask.sum()}")
    ...
    print(f"    Sessions passing filter: {base_mask.sum():,} / {len(sessions):,}")
    ...
    for vid, g in df.groupby('vehicle_id'):
        ...
        # Sensor calibration correction: Coulomb counting + SOC sensor bias causes a
        # systematic ~5-10% underread of implied_Q_Ah even for a healthy pack. When
        # STRICT baseline snaps q_base to the physical nominal (e.g. 608 Ah) but the
        # sensor-measured value at 100% health is only ~566 Ah, the SOH ceiling becomes
        # 93% instead of 100%. We correct by scaling implied_Q_Ah upward by the ratio
        # q_base / q_raw_measured (capped at 15% to guard against bad early data).
        ...
        # Physical validity filter — sessions outside possible range get soh_label=NaN
        # (NOT clipped, because clipping to 100% injects false soh_label=100% into the
        # rolling median which then gets suppressed by monotone enforcement, adding noise).
        #
        # Below 40% of nominal → charge too short or data garbage (e.g. 122 Ah for 306 Ah pack).
        # Above 100% of nominal → impossible; noise from tiny delta_soc (0.5% SOC swing × noise).
        #
        # NaN sessions: rolling median skips them (min_periods=2), then bfill/ffill carries
        # the last good SOH value forward — customer sees previous good reading, not a blank.
        valid_q = (
            (corrected_q >= 0.40 * q_base_for_soh) &
            (corrected_q <= q_base_for_soh)
        )
        raw_soh = (corrected_q / q_base_for_soh * 100).clip(0, 100.0)
        g['soh_label'] = raw_soh.where(valid_q, other=np.nan)
        # Fallback: if too few valid sessions remain, use all sessions (no filter)
        if int(valid_q.sum()) < 3:
            g['soh_label'] = raw_soh
            print(f"    [WARN] {vid}: only {int(valid_q.sum())} sessions within valid implied_Q "
                  f"range [{0.40*q_base_for_soh:.0f}–{q_base_for_soh:.0f} Ah]; using all sessions.")

        # Smooth + enforce monotone decay.
        # smooth_window=21 (was 9) — wider window prevents 2-3 noisy early sessions from
        # permanently anchoring the monotone curve at a pessimistic level.
        g['soh_smooth'] = (
            g['soh_label']
        ...
        min_drop_label = _adaptive_min_drop(g['soh_label'].dropna().values, SOFT_MIN_DROP_LABEL)
