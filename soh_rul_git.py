...
def _apply_confirmation_gate(values,
    ...
    New LOWER level: requires window_down (30) consecutive sessions before accepting.
    New HIGHER level: requires window_up (5) consecutive sessions before accepting.
    During the waiting period the customer sees the last confirmed (good) value.
    Real sustained degradation (30+ sessions consistently lower) is eventually shown.
    Short noise dips (< 30 sessions) are never shown to the customer.
    """
    ...
    for i in range(len(arr)):
        ...
        if abs(cur - confirmed) <= threshold:
            ...
            going_down = cur < confirmed
            window     = window_down if going_down else window_up

            if np.isfinite(pending) and abs(cur - pending) <= threshold:
                pending_count += 1
            else:
                pending       = cur
                pending_count = 1
            ...
            if pending_count >= window:
                ...
                out[i] = confirmed   # show last confirmed to customer
...
def _print_summary_tables(rul_all: dict, replacement_events: pd.DataFrame):
    ...
    if rows:
        ...
        tech_show = pd.DataFrame({
            ...
            'RUL_P50': [
                'New vehicle' if str(b) == 'monitoring_in_progress' else str(r)
...
def compute_all_rul(xgb_results, lstm_results, df_raw, prev_rul_all: dict = None,
    ...
    for vid, res in xgb_results.items():
        ...
        # Use soh_display (XGBoost + 1pp confirmation gate) instead: it is
        # stable enough for customer display (gate suppresses session-to-session
        ...
        if not _span_ok or not _sess_ok:
            ...
            _mo_so_far = (_eff_days_span / 30.44) if np.isfinite(_eff_days_span) else 0.0
            rul['rul_status_note'] = (
                f"New vehicle — SOH ~100%, collecting baseline data "
                f"({_total_sessions} sessions, {_mo_so_far:.1f} months so far)"
            )
            print(f"    [RUL] {vid}: new vehicle — {_why_str} "
        ...
        if _sb == 'monitoring_in_progress':
            ...
            print(f"Est. end-of-life date   : Available after {AH_MODEL_MIN_SESSIONS}+ sessions & 6+ months of history")
...
def plot_customer_views(xgb_results, lstm_results, rul_all, replacement_events, save_path):
    ...
    for vid, res in xgb_results.items():
        ...
        if _slope_basis == 'monitoring_in_progress':
            ...
            ax2.text(0.03, 0.48, f"Est. end-of-life date   : Available after {AH_MODEL_MIN_SESSIONS}+ sessions & 6+ months of history", fontsize=10.2)
