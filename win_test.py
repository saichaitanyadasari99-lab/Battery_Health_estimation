...
def _apply_1pct_hold_gate(values):
    """Hold previous SOH when session-to-session jump exceeds 1 pp.

    Replaces strict global-minimum monotone post-processing for soh_xgb.
    Small gradual changes (<=1 pp either direction) pass through.
    Large jumps (noise, sensor glitch) are suppressed by holding the last
    accepted value until the next well-behaved session arrives.
    """
    arr = np.asarray(values, dtype=float)
    out = arr.copy()
    prev = arr[0] if (len(arr) > 0 and np.isfinite(arr[0])) else np.nan
    for i in range(1, len(arr)):
        cur = arr[i]
        if not np.isfinite(cur):
            out[i] = prev if np.isfinite(prev) else cur
        elif not np.isfinite(prev):
            out[i] = cur
            prev = cur
        elif cur <= prev or (cur - prev) <= 1.0:
            out[i] = cur
            prev = cur
        else:
            out[i] = prev  # hold — upward jump > 1 pp, not trusted
    return out
...
def train_xgboost_soh(labeled: pd.DataFrame) -> dict:
    ...
    for vid, g in tqdm(vehicles, desc='Training XGBoost', unit='vehicle'):
        ...
        if len(X) < 8:
            ...
            g['soh_xgb'] = _apply_1pct_hold_gate(g['soh_smooth'].values)
        ...
        # Predict + post-process with 1pp hold gate (replaces strict monotone)
        raw_pred = model.predict(scaler.transform(X.values))
        g['soh_xgb'] = _apply_1pct_hold_gate(raw_pred)
