...
EXPECTED_CAPACITY_OPTIONS_AH = (104.5, 153.0, 306.0)  # Fleet pack capacities
...
CONFIG_PROFILES = {
    'default': {
        ...
        'MIN_DELTA_SOC': 1.0,
        'MIN_AH': 1.0,
        'EXPECTED_CAPACITY_OPTIONS_AH': [104.5, 153.0, 306.0],
...
def compute_soh_labels(
    ...
    for vid, g in df.groupby('vehicle_id'):
        ...
        valid_q = (
            (g['implied_Q_Ah'] >= 0.40 * q_base_for_soh) &
            (g['implied_Q_Ah'] <= q_base_for_soh)
        )
        raw_soh = (g['implied_Q_Ah'] / q_base_for_soh * 100).clip(0, 100.0)
